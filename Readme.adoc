...
----
. Make sure you don't change the actual code, so that it stays compatible to the orignal tag.
. Commit your changes.
. Tag your change with `{example-version}-for-{example-scala-version}`. This will trigger the `publish-sonatype` workflow in GHA.
